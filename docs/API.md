...
답변 수신·DB 저장 성공 시 200:

```json
{"answer":"안녕하세요! 무엇을 도와드릴까요?"}
```

AI 통신 실패는 상태 코드로 구분한다. 제공자의 오류 상세는 응답에 넣지 않는다.

| 상태 | 상황 | `detail` |
| --- | --- | --- |
| 504 | 제한 시간(`AI_TIMEOUT`) 초과 | AI 응답이 지연되어 답변을 받지 못했습니다. 잠시 후 다시 시도해 주세요. |
| 502 | 제공자 오류·연결 실패·빈 답변 | AI 서비스에 연결하지 못했습니다. 잠시 후 다시 시도해 주세요. |
| 500 | 최근 문맥 조회 실패 | 최근 대화 기록을 불러오지 못했습니다. |
