...
회원가입, 로그인, 현재 사용자 확인, 로그아웃과 채팅·기록 API 및 화면 접근 제어를 구현했다.
답변 수신 후 대화 저장과 본인 기록 조회 API가 연결되어 있다.
최근 5쌍의 문맥 전달과 실제 AI 호출까지 연결되어 있다. 로그인·회원가입·채팅·기록 화면은
각 화면 전용 JavaScript를 통해 API를 호출하며, 성공·오류·세션 만료 상태를 표시한다.
...
| `POST /api/chat` | `question` | 200, AI 답변. 로그인 필요 |
| `GET /api/me/chats` | 본문 없음 | 200, 본인 대화 기록 배열. [DB 안내](DATABASE.md) 참고 |

사용자명은 앞뒤 공백 제거·소문자 변환 후 영문·숫자·밑줄 3~30자로 검사한다.
가입 비밀번호는 8~128자, 로그인 비밀번호는 1~128자를 받는다. 로그인은 가입 정책 대신
...
로그인한 사용자가 `/login` 또는 `/signup`을 열면 303으로 `/chat`에 이동시킨다.
