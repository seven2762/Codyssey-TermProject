...
----------------------------------
## 3. 프론트엔드 독립 로그인 화면 분리 및 채팅 연동 계획 (2026/09/25)

### 3.1. 결정 배경 및 목표
- **방향 확정**: 백엔드의 강제 접근 제어 정책(비로그인 시 `/login` 리다이렉트, 로그인 시 `/chat` 접근 허용)을 전적으로 수용하고, 백엔드의 더미 로그인 템플릿(`login.html`, `signup.html`)을 프론트엔드에서 독립된 정식 화면으로 완성하여 관리한다.
- **핵심 목표**:
  1. `/login` 및 `/signup`을 위한 모던하고 완성도 높은 전용 UI/UX 제공 (`style.css` 기반 카드형 레이아웃).
  2. 프론트엔드에서 로그인 성공 시 백엔드 세션 쿠키를 발급받고, 즉시 백엔드가 관리하는 메인 채팅 화면(`/chat`)으로 안전하게 진입.
  3. `chat.html` 내 중복 모달 코드를 정리하여 단일 책임 원칙 준수 및 유지보수성 향상.

### 3.2. 화면 및 컴포넌트별 상세 작업 내용
1. **로그인 화면 (`backend/app/templates/login.html`)**
   - 백엔드의 더미 텍스트("로그인 화면을 준비 중입니다.") 제거.
   - 중앙 집중형 카드 레이아웃 적용 (아이디, 비밀번호 입력 필드, 로그인 버튼, 에러 메시지 컨테이너, 회원가입 페이지 이동 링크).
   - 반응형 디자인 적용 (모바일/데스크톱 대응).

2. **회원가입 화면 (`backend/app/templates/signup.html`)**
   - 중앙 집중형 카드 레이아웃 적용 (아이디, 비밀번호, 비밀번호 확인 필드, 회원가입 버튼, 에러 메시지 컨테이너, 로그인 페이지 이동 링크).
   - 클라이언트 1차 검증: 아이디 3~30자(영문/숫자/_), 비밀번호 15~128자, 비밀번호 일치 확인.

3. **인증 스크립트 분리 및 API 연동 (`backend/app/static/js/auth_page.js`)**
   - CSRF 헤더 포함 공용 헬퍼(`postJSON`) 활용.
   - `POST /api/login` 처리:
     - 성공 시: `window.location.assign('/chat')`를 호출하여 백엔드가 제어하는 채팅 메인 화면으로 전환.
     - 실패 시: 401(아이디/비번 불일치), 422(검증 실패), 500(서버 오류)별 사용자 친화적 에러 메시지 출력.
   - `POST /api/signup` 처리:
     - 성공 시: 성공 안내 메시지 표시 후 `/login`으로 자동 이동.
     - 실패 시: 409(아이디 중복), 422(입력 검증 오류) 등 필드별 에러 처리.

4. **채팅 화면 및 사이드바 정리 (`chat.html`, `auth_sidebar.js`)**
   - 비로그인 접근 차단은 백엔드(`pages.py`)에서 303으로 보장하므로, `chat.html` 내부의 중복된 `#login-modal`, `#signup-modal`, `#guest-input-overlay` 정리.
   - 세션 만료(401 발생) 시 모달 대신 `window.location.assign('/login')`으로 자연스러운 페이지 이동 유도.
   - 사이드바 하단 로그아웃(`POST /api/logout`) 성공 시 `/login` 화면으로 리다이렉트.

### 3.3. 실행 체크리스트
- [x] `login.html` 템플릿 마크업 및 스타일링 (`style.css` 활용)
- [x] `signup.html` 템플릿 마크업 및 스타일링
- [x] 로그인/회원가입 전용 클라이언트 JS (`auth_page.js`) 구현 및 CSRF 연동
- [x] 백엔드 라우터(`pages.py`) 루트(`/`) 및 `/signup` 세션 직행 처리 최적화
- [x] 로그아웃 시 `/login` 리다이렉트 연동 (`auth_sidebar.js`)
- [ ] 브라우저 환경 통합 검증 및 원자적 커밋 분리